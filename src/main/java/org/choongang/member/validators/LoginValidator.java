...
public class LoginValidator implements Validator<RequestLogin>, RequiredValidator {
    ...
    public void check(RequestLogin form) {
        ...
        //가입된 회원인지 체크
        String msg = "이메일 또는 비밀번호가 일치하지 않습니다";
        Member member = mapper.get(email);
        checkTrue(member != null, new AlertException(msg, status));

        //비밀번호 일치여부 체크
        checkTrue(BCrypt.checkpw(password, member.getPassword()), new AlertException(msg, status));
    }
}
